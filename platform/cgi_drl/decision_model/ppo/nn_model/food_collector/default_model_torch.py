...
class PolicyModel(nn.Module):
    def __init__(self, network_settings):
        ...
        # 84x84 => 21x21
        self.conv = nn.Conv2d(3, 32, kernel_size=8, padding=2, stride=4)
        ...
        self.fc = nn.Linear(64*11*11, 256)
        self.lstm = nn.LSTM(256, self.memory_size // 2)
        ...
        for i_head in range(self.value_head_count):
            self.value_heads.append(nn.Linear(self.memory_size // 2, 1))
        
        self.policy_heads = nn.ModuleList()
        for i_space in self.action_space:
            self.policy_heads.append(nn.Linear(self.memory_size // 2, i_space.n))
