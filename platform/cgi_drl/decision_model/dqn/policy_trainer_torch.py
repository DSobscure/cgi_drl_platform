...
class PolicyTrainer():
    ...
    def update(self, transitions, extra_settings = None):
        ...
        if self.use_rnn:
            network_output = self.bevahior_network(observations, self.rnn_sequence_length, self.rnn_burn_in_length)
            bevahior_q_values = network_output["q_value"]
        else:
            bevahior_q_values = self.bevahior_network(observations)
        ...
        for i_head in range(self.value_head_count):
            ...
            for i_space in range(len(self.action_space)):
                ...
                branch_q_loss = torch.mean(loss_weights * torch.nn.functional.huber_loss(behavior_q, self.invertible_value_function(q_values[:, i_head], is_inverse=False).view(-1, 1), reduction='none'))
    ...
    def get_behavior_q_values(self, observations, extra_settings = None):
        ...
        if self.use_rnn:
            ...
            q_values = self.bevahior_network(observations)
