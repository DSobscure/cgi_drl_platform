...
class EncoderModel():
    ...
    def build_cnn_encoder(self, x, is_train_mode):
        ...
        x = tf.reshape(x, [-1, 64, 32])
        return  x

    def build_cnn_decoder(self, x, is_train_mode):
        x = tf.reshape(x, [-1, 8, 8, 32])
        dcnn_kernel_sizes = [[4,4,16,32],[8,8,3 * self.visula_observation_frame_count,16]]
        dcnn_strides = [(1, 2, 2, 1),(1, 4, 4, 1)]
        batch_size = tf.shape(x)[0]
        dcnn_output_shape = [[batch_size,16,16,16],[batch_size,64,64,3 * self.visula_observation_frame_count]]
