...
class BrawlStarsMatchResultBuffer(object):
    def __init__(self, config):
        ...
        maps = dataset["map"].tolist()
        modes = dataset["mode"].tolist()
        self.map_set = [np.NaN, 'Backyard Bowl', 'Beach Ball', "Belle's Rock", 'Bridge Too Far', 'Canal Grande', 'Center Stage', 'Controller Chaos', 'Crystal Arcade', 'Deep Diner', 'Double Swoosh', 'Dry Season', 'Dueling Beetles', 'Excel', 'Flaring Phoenix', 'Gem Fort', 'Goldarm Gulch', 'Hard Rock Mine', 'Hideout', 'Hot Potato', 'Kaboom Canyon', 'Layer Cake', 'Minecart Madness', 'New Perspective', 'Open Zone', 'Out in the Open', 'Parallel Plays', 'Pinhole Punt', 'Pit Stop', 'Purple Paradise', 'Quarter Pounder', 'Ring of Fire', 'Riverside', 'Safe Zone', 'Shooting Star', 'Snaked Assault', 'Sneaky Fields', 'Split', 'Sunny Soccer', 'Super Beach', 'Temple Ruins', 'Tornado Ring', 'Undermine', 'X Marks the Spot']
        self.mode_set = [np.NaN, 'bounty', 'brawlBall', 'gemGrab', 'heist', 'hotZone', 'knockout']
        ...
        for i_index in range(len(heros1_1)):
            ...
            player1_combo = sorted([heros1_1[i_index], heros1_2[i_index], heros1_3[i_index], 64 + self.map_set.index(maps[i_index]), 64 + 44 + self.mode_set.index(modes[i_index])])
            player2_combo = sorted([heros2_1[i_index], heros2_2[i_index], heros2_3[i_index], 64 + self.map_set.index(maps[i_index]), 64 + 44 + self.mode_set.index(modes[i_index])])
