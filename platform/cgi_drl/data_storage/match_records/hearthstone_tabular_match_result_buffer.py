    ...
    def sample_all_combos(self, batch_size):
        ...
        for i in range(0, 91 * 91, batch_size):
            if i + batch_size >= 91 * 91:
                batch_size = 91 * 91 - i
            yield {
                "player1_combos": [self.encode_combo(match_buffer[j][0]) for j in range(i, i + batch_size, 1)],
                "player2_combos": [self.encode_combo(match_buffer[j][1]) for j in range(i, i + batch_size, 1)],
