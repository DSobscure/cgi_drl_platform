...
def launch(problem_config):
    ...
    for target_trajectory in target_trajectories:
        ...
        for reference_style in reference_styles:
            ...
            playstyle_distance, jaccard_index = compute_similarity(target_style_info, reference_style_info, threshold_count)
            playstyle_distances[reference_style] = playstyle_distance
...
def compute_similarity(style_A, style_B, threshold_count):
    ...
    for s in intersection_state:
        ...
        playstyle_distance += distance
        valid_state_count += 1
    
    if valid_state_count == 0:
        playstyle_distance = math.inf
    else:
        playstyle_distance/= valid_state_count
