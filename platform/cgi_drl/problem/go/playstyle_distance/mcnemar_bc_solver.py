    ...
    for unit_of_query in unit_game_count:
        for unit_of_candidate in unit_game_count:
            ...
            for query_style in query_styles:
                ...
                playstyle_bc_similar_probabilities = {}
                jaccard_indexes = []
                ...
                for candidate_style in candidate_styles:
                    ...
                    bc_distances, jaccard_index, state_space = compute_bc_similarity(query_style_info, candidate_style_info)
                    intersection_state_counts.append(state_space)
                    all_style_distances[candidate_style] = distances
                    all_style_bc_distances[candidate_style] = bc_distances
                ...
                for candidate_style in candidate_styles:
                    ...
                    bc_similar_probability = np.double(0.0)
                    ...
                    for d in all_style_bc_distances[candidate_style]:
                        bc_similar_probability += d / state_space
                    playstyle_bc_similar_probabilities[candidate_style] = bc_similar_probability

                sorted_by_similarity = sorted(playstyle_similar_probabilities.items(), key=lambda d: d[1], reverse=True) 
                sorted_by_bc_similarity = sorted(playstyle_bc_similar_probabilities.items(), key=lambda d: d[1], reverse=True) 
                if query_style == sorted_by_similarity[0][0]:
                    accurate_count += 1       
                if query_style == sorted_by_bc_similarity[0][0]:
                    bc_accurate_count += 1       
                query_count += 1
            ...
            print("Unit of Query: {}, Unit of Candidate: {}, Accuracy: {}, BC Accuracy: {}, p-value: {}".format(unit_of_query, unit_of_candidate, accurate_count / 200, bc_accurate_count / 200, chi2.sf((mcnemar_record[0]-mcnemar_record[1]) ** 2 / (mcnemar_record[0]+mcnemar_record[1] + 0.000001),1)))
            
def calculate_bhattacharyya_coefficient(action_list1, action_list2):
...
def compute_bc_similarity(style_A, style_B):
    intersection_state = style_A["state_set"].intersection(style_B["state_set"])
    state_space = len(style_A["state_set"].union(style_B["state_set"]))
    valid_state_count = 0
    threshold_count = 1
    ...
    distances = []
    for s in intersection_state:
        distance = calculate_bhattacharyya_coefficient(np.asarray(style_A["actions"][s] + style_A["actions"][s]), np.asarray(style_B["actions"][s] + style_B["actions"][s]))
        distances.append(distance)
        valid_state_count += 1
    jaccard_index = valid_state_count / state_space
    return distances, jaccard_index, state_space