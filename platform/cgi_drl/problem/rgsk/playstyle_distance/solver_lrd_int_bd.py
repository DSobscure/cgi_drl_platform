...
def downsampling(xs, code_level):
    if code_level == -2:
        ...
        xs = np.reshape(xs, [-1, 72, 128, 4])
        xs = np.asarray([cv2.resize(x, (9,16)) for x in xs])
        xs = np.asarray(xs/16, dtype=np.int32)
        xs = [np.reshape(x, 9*16*4).tobytes() for x in xs]
    else:
        xs = np.mean(xs, axis=-1)
        xs = np.transpose(xs, [0, 2, 3, 1])
        xs = np.mean(xs, axis=-1)
        xs = np.reshape(xs, [-1, 72, 128])
        xs = np.asarray([cv2.resize(x, (9,16)) for x in xs])
        xs = np.asarray(xs/16, dtype=np.int32)
        xs = [np.reshape(x, 9*16).tobytes() for x in xs]
    return xs
...
def launch(problem_config):
    ...
    result_path += "code-1_dclip"
    with open(result_path + "_playstyle_int_bd.csv", "w", newline="") as distance_csvfile, open(result_path + "_playstyle_int_bd_iou.csv", "w", newline="") as iou_csvfile:
...
def compute_similarity(style_A, style_B, threshold_count):
    ...
    for s in intersection_state:
        if style_A["state_count"][s] < threshold_count or style_B["state_count"][s] < threshold_count:
            continue
        bc = calculate_bhattacharyya_coefficient(np.asarray(style_A["actions"][s] + style_A["actions"][s]), np.asarray(style_B["actions"][s] + style_B["actions"][s]))
        if bc == 0:
            distance = 10
        else:
            distance = -np.log(bc)
