...
class SimpleWinrateSolver(SupervisedLearningTrainer):
    def initialize(self):
    ...
    def eval(self):
        ...
        total_sample_count = self.result_buffer_train.size()
        for records in self.result_buffer_train.sample_all_batch(self.batch_size):
            ...
            for i in range(sample_count):
                ...
                # print(match_result, winvalue_prediction[i], winvalue_prediction[i + sample_count])
                if match_result > 0.501: # win
                    win_count += 1
                    # if winvalue_prediction[i] > 0.501:
                    #     winvalue_accurate_count += 1
                    # if winvalue_prediction[i + sample_count] < 0.499:
                    #     winvalue_accurate_count += 1
                    if winvalue_prediction[i] >= 0 and winvalue_prediction[i + sample_count] >= 0 and winvalue_prediction[i] - winvalue_prediction[i + sample_count] > 0.001:
                        winvalue_accurate_count += 1
                elif match_result < 0.499: # lose
                    lose_count += 1
                    # if winvalue_prediction[i] < 0.499:
                    #     winvalue_accurate_count += 1
                    # if winvalue_prediction[i + sample_count] > 0.501:
                    #     winvalue_accurate_count += 1
                    if winvalue_prediction[i] >= 0 and winvalue_prediction[i + sample_count] >= 0 and winvalue_prediction[i + sample_count] - winvalue_prediction[i] > 0.001:
                        winvalue_accurate_count += 1
                else: # tie
                    tie_count += 1
                    # if winvalue_prediction[i] >= 0.499 and winvalue_prediction[i] <= 0.501:
                    #     winvalue_accurate_count += 1
                    # if winvalue_prediction[i + sample_count] >= 0.499 and winvalue_prediction[i + sample_count] <= 0.501:
                    #     winvalue_accurate_count += 1
                    if winvalue_prediction[i] >= 0 and winvalue_prediction[i + sample_count] >= 0 and abs(winvalue_prediction[i] - winvalue_prediction[i + sample_count]) <= 0.001:
                        winvalue_accurate_count += 1
        
        # winvalue_accuracy = winvalue_accurate_count / (2 * sample_counter)
        # print("win_count, tie_count, lose_count", win_count, tie_count, lose_count)
