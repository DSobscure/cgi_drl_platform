...
def launch(problem_config):
    ...
    for query_style in query_style_infos: 
        ...
        sorted_by_int_similarity = sorted(playstyle_intersection_similarities.items(), key=lambda d: d[1], reverse=True) 
        sorted_by_iou_similarity = sorted(playstyle_jaccrad_indeces.items(), key=lambda d: d[1], reverse=True) 
    ...
    print(f"Playstyle Intersection Similarity - {pd_vs_pis} - p-value: {chi2.sf((pd_vs_pis[0]-pd_vs_pis[1]) ** 2 / (pd_vs_pis[0]+pd_vs_pis[1] + 0.000001),1)}")
    print(f"Playstyle Intersection BC Similarity - {pd_vs_pibcs} - p-value: {chi2.sf((pd_vs_pibcs[0]-pd_vs_pibcs[1]) ** 2 / (pd_vs_pibcs[0]+pd_vs_pibcs[1] + 0.000001),1)}")
    print(f"Playstyle Jaccard Index - {pd_vs_pji} - p-value: {chi2.sf((pd_vs_pji[0]-pd_vs_pji[1]) ** 2 / (pd_vs_pji[0]+pd_vs_pji[1] + 0.000001),1)}")
    print(f"Playstyle Similarity - {pd_vs_ps} - p-value: {chi2.sf((pd_vs_ps[0]-pd_vs_ps[1]) ** 2 / (pd_vs_ps[0]+pd_vs_ps[1] + 0.000001),1)}")
    print(f"Playstyle BC Similarity - {pd_vs_pbcs} - p-value: {chi2.sf((pd_vs_pbcs[0]-pd_vs_pbcs[1]) ** 2 / (pd_vs_pbcs[0]+pd_vs_pbcs[1] + 0.000001),1)}")
