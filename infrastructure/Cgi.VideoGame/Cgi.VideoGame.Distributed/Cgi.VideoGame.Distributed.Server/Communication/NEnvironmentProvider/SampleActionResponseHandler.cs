...
{
    class SampleActionResponseHandler : EnvironmentProviderResponseHandler
    {
        public SampleActionResponseHandler() : base(typeof(SampleActionResponseParameterCode))
        {
        }
        ...
        public override bool Handle(EnvironmentProvider subject, EnvironmentProviderOperationCode operationCode, OperationReturnCode returnCode, Dictionary<byte, object> parameters, string operationMessage, out string errorMessage)
        {
            ...
            {
                object action = parameters[(byte)SampleActionResponseParameterCode.Action];
                subject.SampleActionResponse(returnCode, action, operationMessage);
                return true;
            }
            else
            {
